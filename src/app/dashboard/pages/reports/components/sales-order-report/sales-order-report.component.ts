...
export class SalesOrderReportComponent implements OnInit {
  ...
  units: any[] = [];
  users: any[] = [];
  statusOptions = ['PENDING', 'VETTED', 'APPROVED', 'CLEARED', 'REJECTED', 'CANCELLED', 'COMPLETED'];
  ...
  loadReport(): void {
    ...
    this.reportService.getSalesOrderReport(params).subscribe({
      next: (response) => {
        this.orders = Array.isArray(response) ? response : (response?.data || response?.content || []);
        this.isLoading = false;
      },
      error: (error) => {
        console.error('Error loading sales order report:', error);
        this.errorMessage = 'Failed to load sales order report. Please try again.';
        this.isLoading = false;
        this.orders = [];
      }
    });
