...
@Component({
  selector: 'app-sales-credit-pending',
  templateUrl: './sales-credit-pending.component.html',
  styleUrls: ['./sales-credit-pending.component.css']
})
export class SalesCreditPendingComponent implements OnInit {
  pendingCredits: CreditResponse[] = [];
  bankAccounts: any[] = [];
  loading = false;
  ...
  ngOnInit(): void {
    this.loadPendingCredits();
    this.loadBankAccounts();
  }

  loadPendingCredits(): void {
    this.loading = true;
    this.salesCreditService.getPendingCredits().subscribe({
      next: (res: any) => {
        this.pendingCredits = Array.isArray(res) ? res : (res?.data || res);
        this.loading = false;
      },
      error: (error: any) => {
        this.loading = false;
